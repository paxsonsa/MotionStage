...
use cinemotion_discovery::{DiscoveryAdvertisement, DiscoveryPublisher};
use cinemotion_media::{
    negotiate_stream, NegotiatedVideoStream, SignalingHub, VideoClientCapability,
    VideoStreamDescriptor,
};
use cinemotion_protocol::{
...
use cinemotion_transport_quic::{MotionDatagram, QuicServer};
use cinemotion_webrtc::WebRtcSession;
use thiserror::Error;
    ...
    fn ensure_auth(&self, req: &RegisterRequest) -> Result<(), RejectCode> {
        match self.config.security_mode {
            ...
            SecurityMode::PairingRequired => {
                let expected = self.config.pairing_token.as_deref().unwrap_or("cinemotion");
            ...
            SecurityMode::ApiKey => {
                let expected = self.config.api_key.as_deref().unwrap_or("cinemotion");
            ...
            SecurityMode::ApiKeyPlusPairing => {
                let pair = self.config.pairing_token.as_deref().unwrap_or("cinemotion");
                let key = self.config.api_key.as_deref().unwrap_or("cinemotion");
...
impl ServerHandle {
    ...
    pub async fn stop(&self) -> Result<(), ServerError> {
        ...
        info!(server_name = %name, "cinemotion server stopped");
        Ok(())
    }
                ...
                let mapping = snapshot
                    ...
                    .find(|m| {
                        m.source_device == device_id
                            ...
                            && m.state == cinemotion_core::MappingState::Active
                    })
                    .ok_or_else(|| {
    ...
    pub async fn start_recording(
        ...
        if let Some(recording) = state.active_recording.as_mut() {
            ...
            for mapping in snapshot.mappings.values() {
                if mapping.state == cinemotion_core::MappingState::Active {
                    recording
                        .writer
                        .push_marker(RecordingMarker::MappingCreated {
                            timestamp_ns: now_ns,
    ...
    pub async fn handle_video_signal(
        ...
        match payload {
            SignalPayload::Sdp(sdp) if sdp.ty == SdpType::Offer => {
                ...
                if needs_track {
                    let stream_id = format!("cinemotion-{device_id}");
...
async fn handle_quic_peer(
    server: ServerHandle,
    peer: cinemotion_transport_quic::QuicPeer,
) -> Result<(), ServerError> {
...
async fn send_protocol_error(
    control: &mut cinemotion_transport_quic::ControlChannel,
    code: RejectCode,
    reason: String,
) -> Result<(), ServerError> {
    control
...
mod tests {
    ...
    use cinemotion_core::{AttributeValue, MappingRequest, Scene, SceneAttribute, SceneObject};
    use cinemotion_media::{
    ...
    use cinemotion_recording::{read_recording, RecordingFormatVersion, RecordingMarker};
    use cinemotion_transport_quic::{AttributeUpdateFrame, ControlChannel, QuicClient, QuicPeer};
    use cinemotion_webrtc::WebRtcSession;
    ...
    async fn create_video_offer_creates_server_peer() {
        ...
        let offer = server
            .create_video_offer(device_id, "cinemotion", "camera0")
            .await
            .unwrap();
    ...
    async fn quic_runtime_accepts_session_and_ingests_motion() {
        ...
        peer.send_motion_datagram(cinemotion_transport_quic::MotionDatagram {
            device_id,
            timestamp_ns: 10,
