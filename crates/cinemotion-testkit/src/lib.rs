use cinemotion_protocol::{ClientHello, ClientRole, Feature, RegisterRequest};
use cinemotion_server::{ServerConfig, ServerError, ServerHandle, ServerMetrics};
    ...
    pub async fn bootstrap_motion_client(&self, name: &str) -> Result<Uuid, ServerError> {
        ...
        self.server
            .hello_exchanged(ClientHello {
                protocol_major: cinemotion_protocol::PROTOCOL_MAJOR,
                protocol_minor: cinemotion_protocol::PROTOCOL_MINOR,
                device_id,
        ...
        while started.elapsed() < duration {
            self.server
                .ingest_motion_samples(
                    device_id,
                    vec![cinemotion_core::AttributeUpdate {
                        output_attribute: output_attribute.to_owned(),
                        value: cinemotion_core::AttributeValue::Vec3f([
                            sent as f32,
                            sent as f32,
                            sent as f32,
                        ]),
...
mod tests {
    ...
    use cinemotion_protocol::SessionState;
    ...
    async fn scheduler_counters_progress_at_runtime_start() {
        let mut config = cinemotion_server::ServerConfig::default();
        config.enable_discovery = false;
        config.quic_bind_addr = "127.0.0.1:0".parse().unwrap();
        let server = cinemotion_server::ServerHandle::new(config);
